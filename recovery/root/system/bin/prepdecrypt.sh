...
echo "=== Decrypt preparation ready property set ==="

echo "Waiting for /data to decrypt..."

while [ ! -d /data/media/0/Android ]; do
    sleep 5
done

echo "Watcher: /data is now accessible. Removing .twrps.."
rm -f /persist/.twrps

if [ $? -eq 0 ]; then
    echo "Success! /persist/.twrps removed"
else
